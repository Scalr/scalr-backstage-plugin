...
import { scalrPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [scalrPlugin],
    });

    await request(server).get('/api/scalr/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/scalr/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get(`/api/scalr/todos/${createdTodoItem.id}`)
